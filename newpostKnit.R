## Código adaptado do dwKnit.R (https://github.com/dwoll/RExRepos/blob/master/dwKnit.r)
...
newpostKnit <- function(fIn, fOut,
  ...
  knitr::opts_chunk$set(cache.path=file.path("./tmp/cache/"),
                        fig.path=file.path("../content/static/figure/"),
...
gitHubLinks <- function(fIn, fOut) {
  ...
  # cabeçalho da parte dos links dos códigos
  cabecalho <- readChar("tituloCodGh.txt", 1000)
    fCont[nLines + 1] <- ""
...
postProcess <- function(fIn, fOutPre,
  ...
  writeLines(enc2native(fCont), fIn)                   ## overwrite original file with sanitized one
...
sanitizeMath <- function(fIn, markdEngine=c("pandoc", "kramdown", "redcarpet")) {
  markdEngine <- match.arg(markdEngine)
...
# siteGen     <- args[4]
# 
# newpostKnit(inputFile, outputFile, markdEngine, siteGen)
